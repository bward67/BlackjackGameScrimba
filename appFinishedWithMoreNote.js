...
const renderGame = () => {
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    //! i is 0 and 0 is < cards.length b/c it is 2 so we run the code which is grab the first item in the cards array and add a space behind it and we add 1 to i
    //! the second time we loop through:
    //!  i is 1 and 1 is < cards.length b/c it is 2 so we run the code which is grab the second item in the cards array and add a space behind it and we add 1 to i so i is now 2
    //and now no matter how many times I click the new card button I will add another new card into the array and display it in the Cards: element - so this is not good we must know the sum and if the sum is => 21  - & we are not ALive - we don't give another card and if hasBlackJack is true we don't give another card
  }
